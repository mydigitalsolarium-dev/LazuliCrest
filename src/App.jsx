...
const NAV = [
  ...
  { id:'bodymap',      icon:'👤',  label:'Body Map Symptom Tracker'        },
  ...
  { id:'diet',         icon:'✿',  label:'AI Meal Prep and Log '         },
  { id:'documents',    icon:'🗂',  label:'Documents'       },
  { id:'advocate',     icon:'🫂',  label:'AI Advocate | ADVY'     },
...
const GLOBAL_CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  ::-webkit-scrollbar-thumb{background:rgba(160,100,240,.5);border-radius:4px}
  ...
  @keyframes pulseGlow{0%,100%{box-shadow:0 0 12px rgba(160,100,240,.5)}50%{box-shadow:0 0 28px rgba(160,100,240,.8)}}
  ...
  /* ── Cards — borders brightened so they are visible ─────────── */
  .glass-card{background:rgba(12,5,26,.82);backdrop-filter:blur(20px);border:1px solid rgba(160,100,240,.35);border-radius:20px;box-shadow:0 8px 32px rgba(0,0,0,.55),inset 0 1px 0 rgba(201,168,76,.07);transition:all .22s ease}
  .glass-card:hover{border-color:rgba(201,168,76,.45);box-shadow:0 12px 44px rgba(0,0,0,.65),0 0 28px rgba(160,100,240,.2);transform:translateY(-2px)}
  .glass-card-static{background:rgba(12,5,26,.82);backdrop-filter:blur(20px);border:1px solid rgba(160,100,240,.35);border-radius:20px;box-shadow:0 8px 32px rgba(0,0,0,.5),inset 0 1px 0 rgba(201,168,76,.06);transition:all .4s ease}

  .matriarch-quote{font-family:'Cormorant Garamond',serif;font-style:italic;color:#C9A84C;line-height:1.8;font-size:17px;text-shadow:0 0 8px rgba(201,168,76,.2)}
  .matriarch-tag{text-transform:uppercase;letter-spacing:.4em;font-size:11px;color:rgba(255,255,255,.45);margin-bottom:6px;display:block}

  /* ── Buttons — bigger, readable ─────────────────────────────── */
  .btn{border:none;border-radius:12px;padding:13px 24px;font-weight:600;font-size:15px;cursor:pointer;transition:all .18s;display:inline-flex;align-items:center;gap:8px;letter-spacing:.1px}
  .btn-gold{background:linear-gradient(135deg,#C9A84C,#E8C96B);color:#000;font-weight:700;box-shadow:0 4px 18px rgba(201,168,76,.4),inset 0 1px 0 rgba(255,255,255,.22)}
  .btn-gold:hover{transform:translateY(-2px);box-shadow:0 8px 28px rgba(201,168,76,.55)}
  .btn-gold:active{transform:translateY(0)}
  .btn-ghost{background:rgba(160,100,240,.15);border:1.5px solid rgba(160,100,240,.5);color:#E0C8FF}
  .btn-ghost:hover{background:rgba(160,100,240,.28);border-color:rgba(201,168,76,.5);color:#fff}
  ...
  .btn-subtle{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);color:rgba(240,232,255,.65)}
  .btn-subtle:hover{background:rgba(255,255,255,.12);color:#F0E8FF}

  /* ── Form fields — bigger, brighter borders ──────────────────── */
  .field{background:rgba(255,255,255,.06);border:1.5px solid rgba(160,100,240,.4);border-radius:12px;padding:13px 16px;font-size:15px;color:#F0E8FF;width:100%;outline:none;transition:all .18s;caret-color:#C9A84C}
  .field:focus{border-color:#C9A84C;background:rgba(201,168,76,.06);box-shadow:0 0 0 3px rgba(201,168,76,.12)}
  .field::placeholder{color:rgba(240,232,255,.35)}
  select.field option{background:#0d0520;color:#F0E8FF}
  label{font-size:13px;font-weight:600;color:#C9A84C;display:block;margin-bottom:7px;text-transform:uppercase;letter-spacing:.8px}
  input[type=range]{accent-color:#C9A84C;cursor:pointer;width:100%}
  input[type=checkbox]{accent-color:#A064F0;width:18px;height:18px;cursor:pointer}
  .pill{display:inline-flex;align-items:center;gap:6px;background:rgba(160,100,240,.22);border:1.5px solid rgba(160,100,240,.5);color:#D8AAFF;border-radius:20px;padding:5px 14px;font-size:13px;font-weight:500}

  /* ── Sidebar — brighter nav text ─────────────────────────────── */
  .sidebar{width:260px;background:rgba(5,2,16,.97);backdrop-filter:blur(28px);border-right:1.5px solid rgba(160,100,240,.25);display:flex;flex-direction:column;position:sticky;top:0;height:100vh;overflow-y:auto;z-index:50;flex-shrink:0;box-shadow:4px 0 32px rgba(0,0,0,.6);transition:transform .3s cubic-bezier(.22,1,.36,1)}
  .nav-item{display:flex;align-items:center;gap:11px;width:100%;padding:12px 16px;border-radius:12px;border:1px solid transparent;background:transparent;color:rgba(240,232,255,.6);font-size:14px;font-weight:500;cursor:pointer;transition:all .16s;text-align:left;position:relative}
  .nav-item:hover{background:rgba(160,100,240,.14);color:rgba(240,232,255,.9);border-color:rgba(160,100,240,.28)}
  .nav-item.active{background:linear-gradient(135deg,rgba(160,100,240,.28),rgba(160,100,240,.14));color:#C9A84C;border-color:rgba(201,168,76,.35);font-weight:600}

  .main-content{flex:1;display:flex;flex-direction:column;min-height:100vh;position:relative;z-index:1;min-width:0}
  .page-inner{flex:1;padding:30px 34px;padding-bottom:44px}
  .mobile-topbar{display:none;align-items:center;gap:12px;padding:14px 16px;background:rgba(5,2,16,.97);backdrop-filter:blur(18px);border-bottom:1.5px solid rgba(160,100,240,.2);position:sticky;top:0;z-index:90;flex-shrink:0}
  ...
  .mobile-overlay{position:fixed;inset:0;background:rgba(0,0,0,.65);z-index:99;backdrop-filter:blur(4px)}

  /* ── Stat cards ──────────────────────────────────────────────── */
  .stat-card{background:rgba(12,5,26,.85);border:1.5px solid rgba(160,100,240,.25);border-radius:18px;padding:22px 18px;cursor:pointer;transition:all .22s;position:relative;overflow:hidden}
  .stat-card:hover{transform:translateY(-4px);border-color:rgba(201,168,76,.4);box-shadow:0 14px 44px rgba(0,0,0,.55),0 0 32px rgba(160,100,240,.18)}
  .stats-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:22px}

  /* ── Auth inputs ──────────────────────────────────────────────── */
  .auth-input{background:rgba(255,255,255,.06);border:1.5px solid rgba(160,100,240,.4);border-radius:14px;padding:15px 18px;font-size:16px;color:#F0E8FF;width:100%;outline:none;transition:all .18s;caret-color:#C9A84C}
  .auth-input:focus{border-color:#C9A84C;background:rgba(201,168,76,.06);box-shadow:0 0 0 3px rgba(201,168,76,.12)}
  .auth-input::placeholder{color:rgba(240,232,255,.35)}

  /* ── Diary ───────────────────────────────────────────────────── */
  .diary-notebook{background:linear-gradient(160deg,rgba(18,8,35,.95) 0%,rgba(10,3,22,.98) 100%);border:1.5px solid rgba(160,100,240,.3);border-radius:4px 16px 16px 4px;box-shadow:inset -4px 0 12px rgba(0,0,0,.3),-2px 0 0 rgba(160,100,240,.4);position:relative;overflow:hidden;animation:inkDrop .4s ease}
  .diary-notebook::before{content:'';position:absolute;left:64px;top:0;bottom:0;width:1px;background:rgba(201,168,76,.1)}
  .diary-lines{background-image:repeating-linear-gradient(transparent,transparent 35px,rgba(160,100,240,.07) 35px,rgba(160,100,240,.07) 36px);background-size:100% 36px;background-position:0 44px}
  .diary-textarea{background:transparent;border:none;outline:none;width:100%;min-height:300px;padding:16px 22px 16px 16px;color:rgba(240,232,255,.92);line-height:36px;resize:none;caret-color:#C9A84C;font-size:16px}
  .diary-entry-card{background:rgba(12,5,26,.82);border:1.5px solid rgba(160,100,240,.25);border-radius:14px;padding:20px 24px;transition:all .22s;cursor:pointer}
  .diary-entry-card:hover{border-color:rgba(201,168,76,.35);transform:translateY(-2px);box-shadow:0 8px 28px rgba(0,0,0,.48)}

  .share-card{background:linear-gradient(135deg,rgba(12,5,26,.92),rgba(18,7,38,.96));border:1.5px solid rgba(201,168,76,.3);border-radius:20px;padding:26px;position:relative;overflow:hidden}
  ...
  @media(max-width:768px){
    ...
    .page-inner{padding:18px 16px 36px}

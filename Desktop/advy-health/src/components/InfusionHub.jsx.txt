...
export default function InfusionHub({ data, upd }) {
  ...
  const [showCheckin, setShowCheckin] = useState(null); // appointment id
  ...
  // Check for upcoming infusions within 48 hours
  ...
  const del = id => { if (confirm('Delete this infusion appointment?')) upd('appointments', allAppts.filter(a=>a.id!==id)); };
